...
class DataProcessor:
    ...
    def filter_users(self,min_rating=600):
        try:
            n_ratings = self.rating_df["user_id"].value_counts()
            self.rating_df=self.rating_df[self.rating_df['user_id'].isin(n_ratings[n_ratings>600].index)].copy()
            logger.info("User sucessfully Filtered")
        except Exception as e:
            logger.error("Error while flitering User_Id")
            raise CustomException("Filtering User_Id Failed",e)
    def scale_ratings(self):
        try:
            ...
            logger.info("Error in Scaling")
            raise CustomException("Scaling Failed",e)
    def encode_data(self):
        try:
            ...
            logger.info("Encoding performed sucessfully")
        except Exception as e:
            logger.error("Encoding failed")
            raise CustomException("Encoding was unsucessful",e)
    def split_data(self,test_size=1000,random_state=42):
        try:
            ...
            X = self.rating_df[["user","anime"]].values ## create a array with first col for user and second for anime
            y = self.rating_df["rating"]
            X_train, X_test, y_train, y_test = train_test_split(X, y, 
                test_size=1000,       # same as your manual 1000 rows
                random_state=42,      # ensures reproducibility
                shuffle=True          # default is True
                )
            self.X_train_array = [X_train[: , 0] , X_train[: ,1]] ## first col for user and second for anime
            self.X_test_array = [X_test[: , 0] , X_test[: ,1]]
            self.y_train = y_train
            self.y_test = y_test
            logger.info("Data is sucessfully splitted")
        except Exception as e:
            logger.error("Data Splitting Failed")
            raise CustomException("Data Splitting was unsucessful",e)
    def save_artifacts(self):
        try:
            ...
            for name,data in artifacts.items():
                ...
                logger.info(f"{name} saved successfully in the processed")
